...
for kpic = 1:numPics
    ...
    for e = 1:ensembleSize
        ...
        ratios] = annealingbasic(A_data, nCircles);

        annDataLen = length(annDataEnergies);

        % some initial analyzing: keep count of (best) energies seen so far
    ...
    save(anndf, 'annDataLen', 'enDataPts', 'enDataR', 'enDataEnerg', 'enDataTemps', 'enDataRatios');
